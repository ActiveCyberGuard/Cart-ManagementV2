...
// // ══════════════════════════════════════════════════
// //  MAP  — each active cart now follows a real path (built from
// //  campus locations) instead of jittering randomly, and the marker
// //  glides smoothly between updates (see .leaflet-marker-icon CSS
// //  transition in style.css).
// // ══════════════════════════════════════════════════
// let leafMap, cartMarkers = [];

// // Give every cart a route path (array of {lat,lng}) to travel back & forth on,
// // plus a moving "progress" index/fraction along that path.
...
//   renderMarkers();
//   // Smoother + more frequent updates (paired with CSS transition on the
//   // marker icon) makes movement look like real gliding motion rather than
//   // a robotic jump every few seconds.
//   setInterval(moveSim, 1000);
// }
...
// // Move each active cart a small step further along its predefined path,
// // bouncing back and forth between the two ends — this reads as realistic
// // road-following movement instead of random teleport-like jitter.
// function moveSim() {
//   CARTS.forEach((c) => {
//     if (c.status !== "active") return;
//     if (!c.path || c.path.length < 2) c.path = buildCartPath(c);

//     const STEP = 0.06; // fraction of a segment covered per tick (tweak for speed)
//     c.segProgress += c.direction * STEP;
...
//         c.segIndex += c.direction;
//       }
//       // bounce at either end of the path
...
//     c.lat = a.lat + (b.lat - a.lat) * c.segProgress;
//     c.lng = a.lng + (b.lng - a.lng) * c.segProgress;
//   });

//   // Update existing markers' positions directly (instead of rebuilding all
//   // markers) — combined with the CSS transition this glides smoothly.
//   const active = CARTS.filter((c) => c.status === "active");
//   active.forEach((cart, i) => {
//     if (cartMarkers[i]) cartMarkers[i].setLatLng([cart.lat, cart.lng]);
//   });
...
//     // Live movement update — runs at the same cadence as moveSim() so the
//     // marker glides continuously (CSS transition handles the in-between
//     // frames) and the map gently follows the cart.
...
//       trackMap.panTo([updated.lat, updated.lng], { animate: true, duration: 1, easeLinearity: 0.4 });
